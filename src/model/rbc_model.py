...
class RbcModel:
    def __init__(self, mesh_file_name, TriangleMicromodel, Loss):
        self.mesh_model         = model.mesh_model.MeshModel(mesh_file_name)

        input_shape     = self.mesh_model.state_tensor.shape[1:]
        outputs_count   = 3
        self.triangle_micromodel    = TriangleMicromodel.Create(input_shape, outputs_count)
        self.loss                   = Loss()

        self.init()

    def init(self, initial_position = [0.0, 0.0, 0.0], initial_velocity = [0.0, 0.0, 0.0]):
        self.mesh_model.init(initial_position, initial_velocity)
    ...
    def forward(self, dt = 0.01):
        ...
        '''
        state = self.mesh_model.create_state_tensor()
